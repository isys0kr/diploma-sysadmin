...
Сначала проблему обошли вручную: у Bastion удалили проблемный one-to-one NAT и назначили динамический публичный адрес. После этого SSH сразу заработал. Однако такой способ не подходил для воспроизводимого проекта, потому что после каждого восстановления мог потребоваться ручной ремонт.

Поэтому итоговую схему изменили:
...
Сначала это было исправлено вручную: стандартный сайт Nginx отключили и создали отдельный server block для Zabbix.
...
Первый рабочий обход снова был ручным — перезапуск PHP-FPM после установки пакетов. Позже эту логику перенесли в Ansible.
...
Сначала правило autoregistration было создано вручную через web-интерфейс. После этого все шесть машин успешно появились в Zabbix.
...
Сначала USE Dashboard настраивался вручную через интерфейс Zabbix. Это было удобно, чтобы подобрать нужные items и проверить, как выглядят данные.

После проверки dashboard был создан через API, экспортирован в JSON и сохранён в репозитории:
...
Для проверки воспроизводимости несколько раз выполнялся destructive test.
...
Таким образом восстановление было проверено не только на уровне кода: инфраструктура действительно полностью удалялась и поднималась заново без ручного входа на серверы.
